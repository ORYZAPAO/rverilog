...
| A21 | `lui`命令のALU結果（書き戻し値）が0になる（原因未特定） | 未特定 | **2026-09-13 発見（未調査）**。A20の独立検証中、picorv32スモークテストが依然TIMEOUTすることを確認し調査したところ、A20が修正したsensitivity経路自体は正しく機能している（`cpuregs`書き込み後の再評価は起きている）にもかかわらず、`lui x4, 0x10000`が本来書き込むべき`0x10000000`ではなく`0`を書き込んでいることが判明。A20の修正範囲（メモリ配列の読み出し側sensitivity）とは独立した、LUI命令のデコード・ALU演算・書き戻し値生成のいずれかにある別バグと推定。原因調査は次セッションで継続（推奨着手順11番、継続） |
...
11. picorv32.v にテストベンチ・クロック生成を追加した上でのフル命令実行シミュレーション確認
    ...
    **2026-09-13**: A19マージ済み状態で再実行しても依然TIMEOUT。フォーク（Explore→Plan
    ...
    A20修正後も依然TIMEOUT、`lui`命令の書き戻し値自体が0になる新規バグA21を発見
    （原因未特定、次セッションへ持ち越し）
