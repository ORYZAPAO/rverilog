...
| A16 | 単項リダクション演算子（`&`/`\|`/`^`/`~&`/`~\|`/`~^`）が全て`~`（ビット反転）として誤ってlowingされる（最重要） | `frontend/src/lower.rs`（`lower_unary_op`） | **未対応（2026-08-15発見、対応中）**。`lower_unary_op`が`+`/`-`/`!`/`~`のみ明示的に扱い、それ以外（単項の`&`/`\|`/`^`/`~&`/`~\|`/`~^`＝リダクション演算子）は全て`_ => Ok(UnOp::BitNot)`のcatch-allで`UnOp::BitNot`に丸め込まれていた。`mir::ir::UnOp`には`RedAnd`/`RedNand`/`RedOr`/`RedNor`/`RedXor`/`RedXnor`が定義済みで`logicval.rs`の`reduce_and`/`reduce_or`等の実装も正しいが、frontendから一度も生成されないため未使用のまま死んでいた。最小再現（`reg [1:0] x; wire o = \|x; wire a = &x;`）で`x=2'b00`のとき本来`o=0,a=0`になるべきところ`o=1,a=1`（`~x`のLSBと一致するパターン）になることを確認、picorv32.v内の`&mem_state`/`\|mem_state`（2bitのmem_state FSMが全ゼロ/全ビットチェックに使用）が誤動作し、`mem_done`の算出条件が常に成立しない・誤って成立する状態になっていたことがpicorv32スモークテストの継続的なタイムアウト原因の一つと強く推定される。これはVerilogの基本的な演算子であり、プロジェクト全体でリダクション演算子が一度も正しく動作していなかったことを意味する重大な発見。対応中 |
